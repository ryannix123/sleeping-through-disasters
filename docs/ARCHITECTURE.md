...
| Filestore attachments | Irreplaceable, intermittent | Near-zero | VolSync rsync-tls, PVC-to-PVC over the VAN, every 2 min |
...
| VolSync rsync-tls key | Generated | `volsync-rsync-tls-dst-odoo-data` — created by the destination, copied to the source by playbook 05 |
